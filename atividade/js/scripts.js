...
$('.add-hashtag').keypress(function (event) {
  ...
  $('.btn-avancar').click(function () {
    ...
    $(pagina + " .tabela .add-hashtag").each(function(){
      if($(this).val()){
        var target = $(this).val();
        var layout = '';
        layout += '<a class="hashtag" data-action="removeHashtag">\n';
...
document.getElementById('btPrint').addEventListener('click', function(){
  var resposta = confirm("Após clicar em salvar, aguarde até que o arquivo seja gerado.")
  if(resposta == true){
    console.log('click')
  ...
  html2canvas(document.body).then(function(canvas) {
     var a = canvas; //adm
     var b = a.toDataURL('image/jpg'); 
     
     var doc = new jsPDF('l', 'mm', 'a4', true);
     doc.addImage(b, 'JPG', 0, -10, 300, 300*(9/16), '', 'FAST');
     doc.addPage()
     ...
     $('#pagina2').show();
      html2canvas(document.body).then(function(canvas) {
        ...
        doc.addImage(d, 'JPG', 0, -10, 300, 300*(9/16), '', 'FAST');
        doc.addPage()
        $('#pagina2').hide();
      });    
        $('#pagina3').show();
        ...
        html2canvas(document.body).then(function(canvas) {
          ...
          doc.addImage(f, 'JPG', 0, -10, 300, 300*(9/16), '', 'FAST');
          doc.addPage()
          $('#pagina3').hide();
        });    
        $('#pagina4').show();
        ...
        html2canvas(document.body).then(function(canvas) {
          ...
          doc.addImage(h, 'JPG', 0, -10, 300, 300*(9/16), '', 'FAST');
          doc.addPage()
          $('#pagina4').hide();
        }); 
        $('#pagina5').show();
        ...
        html2canvas(document.body).then(function(canvas) {
          ...
          doc.addImage(j, 'JPG', 0, -10, 300, 300*(9/16), '', 'FAST');
